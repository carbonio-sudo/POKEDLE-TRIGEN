...
def encontrar_fase(cadeia, nome_especie, fase_atual=1):
    if cadeia["species"]["name"] == nome_especie:
        return fase_atual
    for ev in cadeia["evolves_to"]:
        result = encontrar_fase(ev, nome_especie, fase_atual + 1)
...
def calcular_fase(especie):
    ...
    cadeia = requests.get(cadeia_url).json()["chain"]
    nome = especie["name"]
    return encontrar_fase(cadeia, nome)
